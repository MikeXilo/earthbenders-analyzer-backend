...
def calculate_terrain_statistics(srtm_path: str, slope_path: str, aspect_path: str, bounds: Dict[str, float]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not os.path.exists(slope_path):
            logger.error(f"Slope file not found: {slope_path}")
            return {}
            
        if not os.path.exists(aspect_path):
            logger.error(f"Aspect file not found: {aspect_path}")
            return {}
        ...
        slope_masked = slope_data[slope_data != slope_nodata] if slope_nodata is not None else slope_data
        aspect_masked = aspect_data[aspect_data != aspect_nodata] if aspect_nodata is not None else aspect_data
        ...
        aspect_mean = float(np.mean(aspect_masked)) if len(aspect_masked) > 0 else 0
